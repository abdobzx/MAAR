...
Summary:"""
    ...
    def format_rag_prompt(
        ...
        for i, result in enumerate(search_results):
            source_info = f"[Source: {i+1}] Document: {result.document_metadata.get('filename', 'Unknown') if result.document_metadata else 'Unknown'}\n"
...
class ResponseGenerator:
    ...
    async def generate_response_with_fallback(
        ...
        **kwargs
        ...
        # Essayer d'abord le fournisseur principal
        try:
            if hasattr(self.default_provider, 'generate_response'):
                return await self.default_provider.generate_response(messages, stream, **kwargs)
            else:
                # Si c'est un fournisseur du nouveau système
                if stream:
                    return await self.default_provider.generate_stream(
                        [{"role": msg.role, "content": msg.content} for msg in messages],
                        **kwargs
                    )
                else:
                    response = await self.default_provider.generate_text(
                        [{"role": msg.role, "content": msg.content} for msg in messages],
                        **kwargs
                    )
                    return response
            ...
            # Essayer les fournisseurs de fallback
            for fallback_provider in self.fallback_providers:
                try:
                    if hasattr(fallback_provider, 'generate_response'):
                        return await fallback_provider.generate_response(messages, stream, **kwargs)
                    else:
                        # Si c'est un fournisseur du nouveau système
                        if stream:
                            return await fallback_provider.generate_stream(
                                [{"role": msg.role, "content": msg.content} for msg in messages],
                                **kwargs
                            )
                        else:
                            response = await fallback_provider.generate_text(
                                [{"role": msg.role, "content": msg.content} for msg in messages],
                                **kwargs
                            )
                            return response
            ...
            # Si tous les fournisseurs échouent
            raise LLMError(
...
class SynthesisAgent(LoggerMixin):
    ...
    def __init__(self):
        ...
        # Enregistrement des fournisseurs dans le gestionnaire
    ...
    def _get_default_provider(self) -> LLMProvider:
        ...
        if use_sothemaai_only and self.provider_manager.has_provider("sothemaai"):
            sothemaai_provider = self.provider_manager.get_provider("sothemaai")
            if sothemaai_provider:
                return sothemaai_provider
        
        # Vérifier si le fournisseur configuré est disponible dans le gestionnaire
        if self.provider_manager.has_provider(provider_name):
            return self.provider_manager.get_provider(provider_name)
        
        # Sinon, vérifier dans les fournisseurs traditionnels
    ...
    def _get_fallback_providers(self) -> List[LLMProvider]:
        ...
        for provider_name in fallback_order:
            # Vérifier dans le gestionnaire de fournisseurs d'abord
            if (self.provider_manager.has_provider(provider_name) and 
                self.provider_manager.get_provider(provider_name) != self.default_provider):
                fallbacks.append(self.provider_manager.get_provider(provider_name))
            # Puis dans les fournisseurs traditionnels
            elif (provider_name in self.providers and 
    ...
    async def generate_response(
        ...
        try:
            ...
            if query_request.stream:
                ...
                response_text = await self.response_generator.generate_with_fallback(
                    formatted_messages,
                    self.fallback_providers,
                    stream=False
                )
    ...
    async def summarize_document(
        ...
        try:
            ...
            summary = await self.response_generator.generate_response(messages)
